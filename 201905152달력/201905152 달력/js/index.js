...
function getPlan(year, month, date) {
  ...
  for (var dayoffIndex = 0; dayoffIndex < HoliDays.length; dayoffIndex++) {
    ...
    if (selectedDate.getDate() == dateNum) {
      planListUl.innerHTML += `<li><div class="plan-list-item" onclick="loadEvent(${index})"><span class="plan-item-color" style="background-color: red"></span><span class="plan-item-lable">${HoliDays[dayoffIndex].dateName}</span></div></li>`;
    }
  }
  ...
  for (var index = 0; index < userPlanObj.length; index++) {
    ...
    if (selectedDate >= planStart && selectedDate <= planEnd) {
      planListUl.innerHTML += `<li><div class="plan-list-item" onclick="loadEvent(${index})"><span class="plan-item-color" style="background-color: ${userPlanObj[index].planColor}"></span><span class="plan-item-lable">${userPlanObj[index].planTitle}</span></div></li>`;
    }
  }
}
